...
export default function ReportPage() {
  ...
  async function handleDelete() {
    ...
    try {
      let url = "/api/report/selling";
      if (searchMode === "daily")
        url += `?idhr=${monthNames[parseInt(monthIdx)]}/${day}/${year}`;
  ...
  function exportToCSV() {
    const rows = [
      ["Nr", "Date", "Time", "Username", "Profile", "Comment", "Price"],
      ...sales.map((s, i) => [
        i + 1,
        s.date,
        s.time,
  ...
  const totalPrice = useMemo(
    () => sales.reduce((sum, s) => sum + parseInt(s.price || "0"), 0),
    [sales],
  );
  const columns: ColumnDef<SaleRecord>[] = useMemo(
    () => [
      {
        id: "nr",
        header: () => <div className="text-center">Nr</div>,
  ...
  if (loading)
    return <PageSkeleton rows={8} columns={6} showSearch showHeader />;
